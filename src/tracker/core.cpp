...
// Use debug shader to show the results of all intermediate phases
...
SHADER_PROGRAM_T balltrack_shader_2 =
{
    .vertex_source = (char*)vshader_vert,
...
SHADER_PROGRAM_T balltrack_shader_3 =
{
    .vertex_source = (char*)vshader_vert,
...
SHADER_PROGRAM_T balltrack_shader_display =
{
    .vertex_source = (char*)vshader_vert,
...
SHADER_PROGRAM_T balltrack_shader_debug =
{
    .vertex_source = (char*)vshader_vert,
    ...
    .uniforms = {
        ...
        ShaderUniform("pixelwidth3", 1.0f / (float)width3),
    },
    .attribute_names = {"vertex"},
};


SHADER_PROGRAM_T balltrack_shader_fixedcolor =
{
    .vertex_source = (char*)vshader_vert,
...
SHADER_PROGRAM_T balltrack_shader_plain =
{
    .vertex_source = (char*)vshader_vert,
...
SHADER_PROGRAM_T balltrack_shader_diff =
{
    .vertex_source = (char*)vshader_vert,
...
int balltrack_core_init(int externalSamplerExtension, int flipY)
{
    ...
    if (externalSamplerExtension == 0) {
        replace_sampler_string(balltrack_shader_1.fragment_source);
        replace_sampler_string(balltrack_shader_debug.fragment_source);
        replace_sampler_string(balltrack_shader_display.fragment_source);
        replace_sampler_string(balltrack_shader_plain.fragment_source);
        replace_sampler_string(balltrack_shader_diff.fragment_source);
    }
    ...
    rc = balltrack_build_shader_program(&balltrack_shader_debug);
    if (rc != 0)
        return rc;
...
int balltrack_core_process_image(int width, int height, GLuint srctex, GLuint srctype)
{
    ...
    render_pass(&balltrack_shader_debug, input, screen);

    // Set scaling back to linear (3rd texture was already on GL_NEAREST)
    GLCHK(glActiveTexture(GL_TEXTURE1));
    GLCHK(glBindTexture(GL_TEXTURE_2D, rtt_tex1.id));
...
void balltrack_process_buffer(uint8_t* pixelbuffer) {
    ...
    // Map to [-1,1]
    FIELD newField;
    newField.xmin = gxmin / ((float)width) - 1.0f;
    newField.xmax = gxmax / ((float)width) - 1.0f;
    newField.ymin = (2.0f * gymin) / ((float)height) - 1.0f;
    newField.ymax = (2.0f * gymax) / ((float)height) - 1.0f;
    ...
    gxmin = (int)(       (1.0f + field.xmin) * (float)width - 1.0f);
    gxmax = (int)(       (1.0f + field.xmax) * (float)width + 1.0f);
    gymin = (int)(0.5f * (1.0f + field.ymin) * (float)height - 1.0f);
    gymax = (int)(0.5f * (1.0f + field.ymax) * (float)height + 1.0f);
    ...
    bool ballFound = maxR > threshold1 && weight > threshold2;
